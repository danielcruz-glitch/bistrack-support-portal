...
export async function POST(
  ...
  try {
    ...
    const invoiceTotal = safeNumber(invoice.total ?? invoice.total_amount);
    const totalHours = workLogs.reduce(
      (sum, log) => sum + safeNumber(log.hours_worked),
      0
    );
    ...
    const invoiceUpdatePayload: Record<string, string> = {};

    if (invoice.status === "draft") {
      invoiceUpdatePayload.status = "sent";
    }
    ...
    if (updateError) {
      ...
      if (message.includes("email_sent_at")) {
        const fallbackUpdate: Record<string, string> = {};

        if (invoice.status === "draft") {
          fallbackUpdate.status = "sent";
        }
        ...
        if (Object.keys(fallbackUpdate).length > 0) {
          ...
          if (fallbackError) {
            return new NextResponse(
              `Invoice emailed, but failed to update status: ${fallbackError.message}`,
              { status: 500 }
            );
          }
        }
        ...
        return new NextResponse(
          `Invoice emailed, but failed to update invoice record: ${updateError.message}`,
          { status: 500 }
        );
      }
    }
    ...
    return new NextResponse("Failed to email invoice.", { status: 500 });
  }
}